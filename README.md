...
```math
F(x)=\sum_{i=1}^{n-1} 100(x_{i+1}-x_{i}^2)^2 + (1-x_{i})^2 ,\left\lbrace con \left\lbrace x={x_{i},\dots,x_{n}}
```
* la funcion de Rastrigin es definidad como 
```math
F(x)=10n + \sum_{i=1}^{n}x_{i}^2 -Acos(2x_{i}\pi),\left\lbrace con \left\lbrace A=10, \left\lbrace x_{i} \in [-5.12,5.12]
```
2. 
